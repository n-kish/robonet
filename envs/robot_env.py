...
def transform_body(angle_deg, body):
    ...
    # print('ratated vector', rotated_vector)
    # exit()
...
class Joint:
    ...
    def __init__(self, node, body):
        ...
        # print("pos from Joint class before", self.pos)
        # if self.type == 'hinge':
        #     self.axis = vec_to_polar(parse_vec(node.attrib['axis']))
        if self.local_coord:
            self.pos += body.pos
        # print("pos from Joint class after", self.pos)
    ...
    def sync_node(self, new_pos):
        if new_pos is not None:
            ...
            pos = self.pos
        # print("pos changed again here in sync_node method of Joint", pos)
...
class Body:
    ...
    def __init__(self, node, parent_body, robot, cfg):
        ...
        # print("pos from body class after changes", self.pos)
...
def graph_to_robot(graph, xml_dir, log_dir):
    ...
    node_map = {0: {'length_ratio':0.05},
                 1: {'length_ratio':0.025},
                 2: {'length_ratio':0.075}
                 }
    ...
    # print("node_cat_id, _edge_cat_id", node_cat_id, _edge_cat_id)

    # print("node_cat_map", node_cat_map)
    # mapping nodes and cat_id
    body_node_map = dict(zip(xml_robot.bodies[init_bodies_count::4], bfs_list))
...
def graph_to_robot_with_init_design(graph, xml_dir, log_dir, exp_method, min_resource, flag):
    ...
    # print("THIS IS THE GRAPH", graph, type(graph))
