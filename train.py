...
from data.sampling_iterator import SamplingIterator
from envs.graph_building_env import GraphActionCategorical, GraphBuildingEnv, GraphBuildingEnvContext
...
class GFNTrainer:
    def __init__(self, hps: Dict[str, Any], device: torch.device):
        ...
        self.task: GFNTask                                  #---->>>>> has mol input at compute_flat_rewards method
        ...
        # `self.sampling_model`.
        #self.offline_ratio = self.hps.get("offline_ratio", 0.5)  ----->>>>> modified by K from 0.5 to 0 to make it online only.
    ...
    def build_final_data_loader(self) -> DataLoader:
        # print("COUNTING ONCE FOR FINAL ROBOT")
        model, dev = self._wrap_for_mp(self.sampling_model, send_to_device=True)
        iterator = SamplingIterator(
            self.training_data,
            model,
    ...
    def train_batch(self, batch: gd.Batch, epoch_idx: int, batch_idx: int, train_it: int) -> Dict[str, Any]:
        ...
        while iter < self.hps["epochs"]:       # this is actual epoch length
            try:
                ...
                raise e
            iter += 1
        # print("train batch by compyting losses took %s seconds" % (time.time() - start))
    ...
    def _save_state(self, it):
        torch.save(
            ...
            open(pathlib.Path(self.hps["log_dir"]) / f"policies/model_state_{it}.pt", "wb"),
        )
    ...
    def log(self, info, index, key):
        ...
        if not hasattr(self, "_summary_writer"):
            self._summary_writer = torch.utils.tensorboard.SummaryWriter(self.hps["log_dir"])
        # print("info from train.py" , info)
        ## put you metrics into tensorboard
