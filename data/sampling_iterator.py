...
# from gflownet_test.envs.robot_env import graph_to_robot
from envs.robot_env import graph_to_robot_with_init_design
# from gflownet_old.envs.robot_env_costaware import graph_to_robot_with_init_design
...
class SamplingIterator(IterableDataset):
    ...
    def __init__(
        ...
        device,
        xml_dir,
        ratio: float = 0,                        #changed to 0 from 0.5 - Kishan
        stream: bool = True,                     #used to be True, changed to ensure we dont consider offline data - Kishan
        ...
        self.log_robots = not hasattr(self.ctx, "not_a_robot_env")  # TODO: make this a proper flag
        self.is_final_sample = final_sample
    ...
    def __iter__(self):
        ...
        if self.log_dir is not None:
            ...
            self.log_path = f"{self.log_dir}/generated_robots_{self._wid}.db"
        ...
        for idcs in self._idx_iterator():
            ...
            if self.sample_cond_info:                   
                ...
                robots, flat_rewards = map(list, zip(*[self.data[i] for i in idcs])) if len(idcs) else ([], [])
                flat_rewards = (
                    list(self.task.flat_reward_transform(torch.stack(flat_rewards))) if len(flat_rewards) else []
                )
                graphs = [self.ctx.mol_to_graph(m) for m in robots]
                ...
                # print("new cond_info, no offline data anymore")
                trajs, flat_rewards = [], []
            ...
            if num_online > 0:
                with torch.no_grad():
                    # print("ENTERED TO GET TRAJ", num_online, num_offline)
                    trajs += self.algo.create_training_data_from_own_samples(
                        self.model,
                        self.xml_dir,
                        self.log_dir,   # adding to redirect robots based on stage (train or valid or final)
                        self.is_final_sample,
                        num_online,
                        self.exp_method,
                        self.min_resource,
                ...
                if self.algo.bootstrap_own_reward:
                    ...
                    online_flat_rew, m_is_valid = self.task.compute_flat_rewards(xml_robots, [trajs[i]["result"] for i in valid_idcs], self.rl_timesteps)    # trajs[i]["result"] - gfn graphs
                    assert (
                    ...
                    # Override the is_valid key in case the task made some robots invalid
                    for i in range(num_online):
                        trajs[num_offline + i]["is_valid"] = is_valid[num_offline + i].item()                    
                    
                    if self.log_robots:
                        for i, m in zip(valid_idcs, valid_robots):
                            trajs[i]["robot"] = m
            ...
            # if num_online > 0 and self.log_dir is not None:        # Turning the collection of robots into a SQL db off - Kishan
            #     self.log_generated(
    ...
    #     if self.log_robots:
    #         robots = [
    #                 (graph_to_robot(trajs[i]["result"], self.xml_dir, self.log_dir)) if trajs[i]["is_valid"] else ""
    #                 for i in range(len(trajs))
    #                 ] 
    #     else:
    #         robots = [nx.algorithms.graph_hashing.weisfeiler_lehman_graph_hash(t["result"], None, "v") for t in trajs]

    #     flat_rewards = flat_rewards.reshape((len(flat_rewards), -1)).data.numpy().tolist()
    #     rewards = rewards.data.numpy().tolist()
    #     preferences = cond_info.get("preferences", torch.zeros((len(robots), 0))).data.numpy().tolist()
    #     focus_dir = cond_info.get("focus_dir", torch.zeros((len(robots), 0))).data.numpy().tolist()
    ...
    #     robot = [robots[i] for i in range(len(trajs))]

    #     print("robots", robot, type(robot), len(robot))
