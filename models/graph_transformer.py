...
from envs.graph_building_env import GraphActionCategorical, GraphActionType
...
class GraphTransformerGFN(nn.Module):
    ...
    def _action_type_to_mask(self, t, g):
        # print("temp", g)
        # if t == GraphActionType.Stop:
        # # Prevent masking for Stop action by returning a mask of ones
        #     return torch.ones((1, 1), device=g.x.device)
    ...
    # def _action_type_to_logit(self, t, emb, g):
    #     logits_shape = self.mlps[t.cname](emb[self._action_type_to_graph_part[t]]).shape
    #     # print("shape defined")
    #     iteration = 0
    #     if t == GraphActionType.Stop:
    #         # print("into stop rec.")
    #         if iteration <= self.data_collection_iters:
    #             # Generate random logits for the first 10 iterations
    #             # print(f"Iteration {iteration}: Using random logits for action type {t}")
    #             logits = torch.empty(logits_shape, device=g.x.device)  # Random logits in range [0, 1] with the correct shape
    #             logits.fill_(random.uniform(0.9, 1.8))          
    #             # print("new  logits", logits)
    #         else:
    #         # After 10 iterations, use the MLP to compute the logits
    #             logits = self.mlps[t.cname](emb[self._action_type_to_graph_part[t]])

    #     else:
    #         # print("rec. non stop")
    #         logits = torch.rand(logits_shape, device=g.x.device)  # Random logits in range [0, 1] with the correct shape
    #         # logits = self.mlps[t.cname](emb[self._action_type_to_graph_part[t]])
    #     # print("logits -> ", logits)

    #     # print(f"logits for {t}", logits)

    #     return self._mask(logits, self._action_type_to_mask(t, g))

    def _mask(self, x, m):
        # mask logit vector x with binary mask m, -1000 is a tiny log-value
        # print(f"The mask value for logit vector: {x} and binary mask: {m} is", x * m + -1000 * (1 - m))
        return x * m + -1000 * (1 - m)
