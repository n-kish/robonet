...
class Graph(nx.Graph):
    # This is the parent Graph being created. All graph modifications are applied on a graph like this. 
    ...
    def __repr__(self):
        ...
        self.node_categories = [self.nodes[i]["v"] for i in self.nodes]
        self.edge_categories = list(self.edges[(src, tgt)][k] for src, tgt in self.edges for k in self.edges[(src, tgt)])
        # print("node_cats", self.node_categories)
        # print("edge_cats", self.edge_categories)

        # print("*********************************************************************************************")

        # print("********length of attr", len(list(self.edges[(src, tgt)][k] for src, tgt in self.edges for k in self.edges[(src, tgt)])))
        # print("THIS IS SELF.NODES", self.nodes[0], type(self.nodes[0]))
        # print("self.nodes[i][""] for i in self.nodes", list(self.nodes[i]["v"] for i in self.nodes))    self.node_categories = [self.nodes[i]["v"] for i in self.nodes]
        return f'<{list(self.nodes)}, {list(self.edges)}, {list(self.edges[(src, tgt)][k] for src, tgt in self.edges for k in self.edges[(src, tgt)])}'   # {list(self.edges[(src, tgt)][k] for src, tgt in self.edges for k in self.edges[(src, tgt)])}>
    
        # gp.edges[(action.source, action.target)][action.attr] = action.value


# class RoboGraph(Graph):
#     #Not implemented
#     def graph_info(self):
#         return self.nodes, self.edges, self.node_categories
...
class GraphAction:
    def __init__(self, action: GraphActionType, source=None, target=None, value=None, attr=None, relabel=None):
        ...
        # print("self.action, self.source, self.target, self.attr, self.value, self.relabel", self.action, self.source, self.target, self.attr, self.value, self.relabel)
...
class GraphBuildingEnv:
    ...
    def step(self, g: Graph, action: GraphAction) -> Graph:
        ...
        action: GraphAction
            ...
            action now contains - action: GraphActionType, source, target, value, attr, relabel
        ...
        if action.action is GraphActionType.AddEdge:
            ...
            if len(g) == 0:
                ...
                # print("BEFORE add_node", gp, type(gp))
                gp.add_node(e[1], v=action.value)
                gp.add_edge(*e)

        elif action.action is GraphActionType.SetNodeAttr:
            # print("SetNodeAttr called ???")
            # print("SET NODE ATTR : action.action, action.source, action.target, action.attr, action.value, action.relabel", action.action, action.source, action.target, action.attr, action.value, action.relabel)
            ...
            # print("SET EDGE ATTR : action.action, action.source, action.target, action.attr, action.value, action.relabel", action.action, action.source, action.target, action.attr, action.value, action.relabel)
            ...
            gp.edges[(action.source, action.target)][action.attr] = action.value
            # print("LOOK WHAT I GOT: LHS RHS", gp.edges[(action.source, action.target)][action.attr], action.value)
            # print("curent graph", gp)
        ...
        # print("gp", gp)
        # print("gpnodes", gp.nodes)
        # print("gpedges", gp.edges, type(gp.edges))
        # if len(gp.edges)>1: 
        #     print("gpedges_0", gp.edges[0])
...
class GraphActionCategorical:
    ...
    def _compute_batchwise_max(
        ...
        # print("maxl", maxl, type(maxl), len(maxl))
        return maxl
    ...
    def sample(self, gum_beta) -> List[Tuple[int, int, int]]:
        ...
        ######### Block of gumbel before bringing in updated safe_gumbel
        # print("COLLECTIN NEW SAMPLE")
        ...
        # # print("GUMBEL", gumbel)
        # # Take the argmax
        # print("Sampled action", self.argmax(x=gumbel))
        ...
        # print("gum_beta is", gum_beta)

        # self.logits[0] = torch.tensor([[10.0]])  # Increase logit for graph-level action


        beta = gum_beta

        # beta = 1       #removing the effects of gum_beta

        # print("self.logits", self.logits)
        u = [torch.rand(i.shape, device=self.dev) for i in self.logits] #uniform noise
        # print("elem in u", u)
        # Gumbel noise
        gumbel = [logit - (-beta * noise.log()).log() for logit, noise in zip(self.logits, u)]
        ...
        # print("gumbel", gumbel)
        # Take the argmax
        # print("this is what sample returns", self.argmax(x=gumbel))
        # print("after argmax", self.argmax(x=gumbel))
    ...
    def argmax(
        ...
        #In simple terms, there are three tensors which are presented to argmax. From these three tensors, 
        #the index of the tensor with the highest value becomes action_type, the  
        ...
        # print("type_max_val, type_max_idx", type_max_val, type_max_idx)
        # print("Batch", batch)
