...
class FragMolBuildingEnvContext(GraphBuildingEnvContext):
    ...
    def __init__(self, max_frags: int = 9, num_cond_dim: int = 0, fragments: List[Tuple[str, List[int]]]  = None):
        ...
        """
        self.max_frags = max_frags  # max_nodes
        if fragments is None:
            # smi, stems = zip(*bengio2021flow.Categories)
        ...
        # print("self.num_stem_acts", self.num_stem_acts)
        self.action_map = [
            (fragidx, stemidx)
        ...
        print("self.action_map", self.action_map)
        ...
        self.action_type_order = [GraphActionType.Stop, GraphActionType.AddNode, GraphActionType.SetEdgeAttr]   # changed GraphActionType.SetEdgeAttr to GraphActionType.SetNodeAttr - Kishan
        # # print("self.action_type_order", self.action_type_order)
    ...
    def aidx_to_GraphAction(self, g: gd.Data, action_idx: Tuple[int, int, int], fwd: bool = True):
        ...
        if t is GraphActionType.Stop:
            ...
            # print("*****************attr & val****************", attr, val)    # this val has site info - Kishan
    ...
    def graph_to_Data(self, g: Graph) -> gd.Data:
        ...
        """
        # print("graph at graph_to_data stage", g)
        ...
        # print("edge attr mask", set_edge_attr_mask)
        ...
        if len(g):
            ...
            # print("degrees, max_degrees", degrees, max_degrees)
        else:
        ...
        for i, e in enumerate(g.edges):
            ed = g.edges[e]
            # print("i and e from enumerated edges", i, e)
    ...
    #     """
    #     #Input - gfn graph | Return - Mujoco Robot Graph

    #     # for i, val in enumerate(self.frags_numatm):
    #     #     print (i, ":",val)
    #     print("Current graph", g , type(g))
    #     # print("g.nodes", g.nodes)
    #     # print("self.frags_numatm[g.nodes[i][""]]", self.frags_numatm[g.nodes[0]["v"]])

    #     offsets = np.cumsum([0] + [self.frags_numatm[g.nodes[i]["v"]] for i in g])
    #     # print("offsets", offsets)
    #     mol = None
    #     #converting each node to equivalent SMILES string from FRAGMENTS and combining them. 
    #     for i in g.nodes:
    #         if mol is None:
    #             mol = self.frags_mol[g.nodes[i]["v"]]
    #             # print("IN IF, mol is", mol)
    #         else:
    #             # adding new SMILES string to the already existing one to extend the mol.
    #             mol = Chem.CombineMols(mol, self.frags_mol[g.nodes[i]["v"]])
    #             # print("IN else, mol from RDKIT chem is", mol)

    #     mol = Chem.EditableMol(mol)
    #     # print("CHEM.editablemol", mol)
    #     bond_atoms = []
    #     for a, b in g.edges:
    #         # print("a,b in g.edges", a,b)
    #         afrag = g.nodes[a]["v"]
    #         bfrag = g.nodes[b]["v"]
    #         # print("afrag, bfarg", afrag, bfrag)
    #         u, v = (
    ...
    #         # print("u,v", u,v)
    #         bond_atoms += [u, v]
    #         # print("BOND ATOMS LIST CHANGED TO", bond_atoms)
    #         mol.AddBond(u, v, Chem.BondType.SINGLE)
    #         # print("after adding bond", mol)
    #     mol = mol.GetMol()
    #     # print("mol.getmol()", mol)

    #     def _pop_H(atom):
    #         # print("POP atom is called")
    ...
    def is_sane(self, env, g: Graph, xml_dir: str, log_dir: str, exp_method, min_resource) -> bool:
        ...
        try:
            ...
            print(f"XML robot of {g} failed because: {e}")
            return False
        # print("returning from is_sane")
        return True
